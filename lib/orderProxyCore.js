...
/**
 * DigiKey MyLists：GET /mylists/v1/lists（官方 Swagger basePath /mylists/v1）
 * 回傳 ListData[]，含 Id、ListName 等；需於 Developer Portal 為應用程式開通 MyLists API。
 * @param {URLSearchParams} searchParams 可選 limit（每頁筆數，預設 50，最大 100）、maxPages（預設 20）
 */
async function handleDigikeyMyLists(searchParams) {
  const miss = requireEnvKeys(['DIGIKEY_CLIENT_ID', 'DIGIKEY_CLIENT_SECRET', 'DIGIKEY_ACCOUNT_ID']);
  if (miss) return { status: 500, body: miss };
  ...
  const maxPagesRaw = parseInt(searchParams.get('maxPages') || '20', 10);
  const maxPages = Number.isNaN(maxPagesRaw) ? 20 : Math.min(50, Math.max(1, maxPagesRaw));
  ...
  for (let page = 0; page < maxPages; page++) {
    const url =
      ...
      '?startIndex=' +
      encodeURIComponent(String(startIndex)) +
      '&limit=' +
    ...
    if (!r.ok) {
      return {
        status: r.status,
        body: { ok: false, upstream: 'digikey', step: 'mylists', status: r.status, body: r.body },
      };
    }

    const chunk = Array.isArray(r.body) ? r.body : [];
    for (let i = 0; i < chunk.length; i++) allLists.push(chunk[i]);
