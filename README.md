...
## 訂料追蹤（Mouser / DigiKey）
金鑰**不要**提交到 repo；可擇一或並存：

### Mouser 訂單欄位
- **ByDateRange** 回傳多為摘要；若需品項／數量等，官方另有 **`GET .../orderhistory/webOrderNumber`**（依 Web Order 取明細）。  
- 同步腳本與即時代理（帶 `enrichDetails=1`）會自動合併明細至快取／回應。

### 方式 0：GitHub Actions + 靜態 JSON（**GitHub Pages 首選**，不必 Vercel）
1. 在 repo **Settings → Secrets and variables → Actions** 新增：`MOUSER_API_KEY`、`DIGIKEY_CLIENT_ID`、`DIGIKEY_CLIENT_SECRET`、`DIGIKEY_ACCOUNT_ID`（與 `.env.example` 相同；可選 `MOUSER_API_VERSION`）。
2. 到 **Actions → Sync order cache → Run workflow** 手動跑一次（或等每 6 小時排程）。
3. Workflow 會將最近 **60 天**訂單寫入 **`data/order-cache/mouser.json`、`digikey.json`**，並同步寫入 **DigiKey MyList 清單**（**`digikey-mylists.json`**），一併 push；**`*.github.io` 開頁時預設讀此快取**（非即時，依 Actions 排程，例：每 6 小時）。
4. 本機產生快取：`npm run sync-order-cache`（需先準備 `.env`）。

### DigiKey MyList 名稱（靜態快取，與訂單同一排程）
- 「訂料追蹤」→「專案 ↔ 訂單對照」→ **讀取 MyList 名稱**：在 **GitHub Pages 預設（靜態快取）**下會讀 **`data/order-cache/digikey-mylists.json`**，**不必** Vercel／本機代理。
- Actions 與訂單共用 **`DIGIKEY_*`** Secrets；請在 [DigiKey Developer Portal — MyLists](https://developer.digikey.com/products/mylists/mylists) 為同一應用程式**開通 MyLists API**（與 Order Status 可能分屬不同產品訂閱）。
- 若改 **即時**讀 MyList（`config.js` 設 **`ORDER_USE_STATIC_CACHE: false`**），則改走後端 **`GET /api/digikey/mylists`**，需 **Vercel／本機代理** 並設定 **`ORDER_PROXY_BASE_URL`**（見下方「方式 A／B」）。
...
**除錯**：若瀏覽器顯示 `Failed to fetch`，請用新分頁開 `https://你的-Vercel-網址/api/health`，應回傳 `{"ok":true,"service":"order-proxy"}`。若打不開，代表代理網址錯或尚未部署。**不要用 https 的 GitHub Pages 去連 `http://localhost` 代理**（瀏覽器會阻擋混合內容）。

> **GitHub Pages**：預設使用 **方式 0** 靜態快取，無需 Vercel。若改為即時查詢，請在 `config.js` 設 **`ORDER_USE_STATIC_CACHE: false`**，並部署 Vercel（或本機代理）並設定 `ORDER_PROXY_BASE_URL`。
