...
3. Workflow 會將最近 **60 天**訂單寫入 **`data/order-cache/*.json`** 並 push；**`*.github.io` 開頁時預設讀此快取**（非即時，依 Actions 排程，例：每 6 小時）。
4. 本機產生快取：`npm run sync-order-cache`（需先準備 `.env`）。

### DigiKey MyList 名稱（選用）
- 「訂料追蹤」→「專案 ↔ 訂單對照」區可 **讀取 MyList 清單名稱**，後端路由 **`GET /api/digikey/mylists`**（與訂單共用 `DIGIKEY_*` 環境變數；須有 **Vercel／本機代理**，GitHub Pages 靜態站無法直接打 DigiKey）。
- 請在 [DigiKey Developer Portal — MyLists](https://developer.digikey.com/products/mylists/mylists) 為同一應用程式**開通 MyLists API**（與 Order Status 可能分屬不同產品訂閱）。
- **若出現「無法連線到代理」**：程式會依倉庫名推測 `https://倉庫-slug.vercel.app`；若您**從未在 Vercel 用本 repo 部署**，該網址不存在（新分頁開 `/api/health` 會失敗）。請先依下方「方式 A：Vercel」部署，再把實際網址填入 **`config.js` 的 `ORDER_PROXY_BASE_URL`**。
