...
4. **`config.js` 的 `ORDER_PROXY_BASE_URL`**：
   - **GitHub Pages + API 在 Vercel**：設為上述 `https` 網址（勿結尾 `/`）。
   - **整站同專案都在 Vercel**：可留空，在 `*.vercel.app` 會自動用目前網址當代理。
...
> **GitHub Pages 僅靜態**：必須設定 `ORDER_PROXY_BASE_URL` 指向已部署的 Vercel 代理網址。
